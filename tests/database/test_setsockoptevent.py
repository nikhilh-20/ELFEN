...
from analysis.analysis_models.dynamic_analysis import SetSockOptEvent, KernelTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class SetSockOptEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def setUpTestData(cls):
        ...
        SetSockOptEvent.objects.create(
            sample=cls.sample,
            kernel_trace=cls.kernel_trace,
            procname=b"test"
        )

    def test_setsockoptevent_get(self):
        """
        This test checks if created SetSockOptEvent object can be retrieved
        successfully from the DB.
        """
        event = SetSockOptEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_setsockoptevent_update(self):
        """
        This test checks if created SetSockOptEvent object can be updated
        successfully.
        """
        event = SetSockOptEvent.objects.get(sample=self.sample)
    ...
    def test_setsockoptevent_delete(self):
        """
        This test checks if an existing SetSockOptEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this SetSockOptEvent object.
        """
        event = SetSockOptEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            SetSockOptEvent.objects.get(sample=self.sample)
            self.fail('SetSockOptEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_setsockoptevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the SetSockOptEvent
    ...
    def test_setsockoptevent_foreignkey_KernelTrace_delete(self):
        """
        This test checks if the KernelTrace object referenced by the SetSockOptEvent
