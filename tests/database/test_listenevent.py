...
from analysis.analysis_models.dynamic_analysis import ListenEvent, KernelTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class ListenEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def test_listenevent_get(self):
        """
        This test checks if created ListenEvent object can be retrieved
        successfully from the DB.
        """
        event = ListenEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_listenevent_update(self):
        """
        This test checks if created ListenEvent object can be updated
        successfully.
        """
        event = ListenEvent.objects.get(sample=self.sample)
    ...
    def test_listenevent_delete(self):
        """
        This test checks if an existing ListenEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this ListenEvent object.
        """
        event = ListenEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            ListenEvent.objects.get(sample=self.sample)
            self.fail('ListenEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_listenevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the ListenEvent
    ...
    def test_listenevent_foreignkey_KernelTrace_delete(self):
        """
        This test checks if the KernelTrace object referenced by the ListenEvent
