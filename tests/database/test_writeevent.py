...
from analysis.analysis_models.dynamic_analysis import WriteEvent, KernelTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class WriteEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def setUpTestData(cls):
        ...
        WriteEvent.objects.create(
            sample=cls.sample,
            kernel_trace=cls.kernel_trace,
            procname=b"test"
        )

    def test_writeevent_get(self):
        """
        This test checks if created WriteEvent object can be retrieved
        successfully from the DB.
        """
        event = WriteEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_writeevent_update(self):
        """
        This test checks if created WriteEvent object can be updated
        successfully.
        """
        event = WriteEvent.objects.get(sample=self.sample)
    ...
    def test_writeevent_delete(self):
        """
        This test checks if an existing WriteEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this WriteEvent object.
        """
        event = WriteEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            WriteEvent.objects.get(sample=self.sample)
            self.fail('WriteEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_writeevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the WriteEvent
    ...
    def test_writeevent_foreignkey_KernelTrace_delete(self):
        """
        This test checks if the KernelTrace object referenced by the WriteEvent
