...
from analysis.analysis_models.dynamic_analysis import StrncpyEvent, UserlandTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class StrncpyEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def setUpTestData(cls):
        ...
        StrncpyEvent.objects.create(
            sample=cls.sample,
            userland_trace=cls.userland_trace,
            procname=b"test"
        )

    def test_strncpyevent_get(self):
        """
        This test checks if created StrncpyEvent object can be retrieved
        successfully from the DB.
        """
        event = StrncpyEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_strncpyevent_update(self):
        """
        This test checks if created StrncpyEvent object can be updated
        successfully.
        """
        event = StrncpyEvent.objects.get(sample=self.sample)
    ...
    def test_strncpyevent_delete(self):
        """
        This test checks if an existing StrncpyEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this StrncpyEvent object.
        """
        event = StrncpyEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            StrncpyEvent.objects.get(sample=self.sample)
            self.fail('StrncpyEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_strncpyevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the StrncpyEvent
    ...
    def test_strncpyevent_foreignkey_userlandtrace_delete(self):
        """
        This test checks if the UserlandTrace object referenced by the StrncpyEvent
