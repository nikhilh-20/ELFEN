...
from analysis.analysis_models.dynamic_analysis import GetPidEvent, KernelTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class GetPidEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def setUpTestData(cls):
        ...
        GetPidEvent.objects.create(
            sample=cls.sample,
            kernel_trace=cls.kernel_trace,
            procname=b"test"
        )

    def test_getpidevent_get(self):
        """
        This test checks if created GetPidEvent object can be retrieved
        successfully from the DB.
        """
        event = GetPidEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_getpidevent_update(self):
        """
        This test checks if created GetPidEvent object can be updated
        successfully.
        """
        event = GetPidEvent.objects.get(sample=self.sample)
    ...
    def test_getpidevent_delete(self):
        """
        This test checks if an existing GetPidEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this GetPidEvent object.
        """
        event = GetPidEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            GetPidEvent.objects.get(sample=self.sample)
            self.fail('GetPidEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_getpidevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the GetPidEvent
    ...
    def test_getpidevent_foreignkey_KernelTrace_delete(self):
        """
        This test checks if the KernelTrace object referenced by the GetPidEvent
