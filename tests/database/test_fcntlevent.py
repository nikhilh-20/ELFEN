...
from analysis.analysis_models.dynamic_analysis import FcntlEvent, KernelTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class FcntlEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def setUpTestData(cls):
        ...
        FcntlEvent.objects.create(
            sample=cls.sample,
            kernel_trace=cls.kernel_trace,
            procname=b"test"
        )

    def test_fcntlevent_get(self):
        """
        This test checks if created FcntlEvent object can be retrieved
        successfully from the DB.
        """
        event = FcntlEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_fcntlevent_update(self):
        """
        This test checks if created FcntlEvent object can be updated
        successfully.
        """
        event = FcntlEvent.objects.get(sample=self.sample)
    ...
    def test_fcntlevent_delete(self):
        """
        This test checks if an existing FcntlEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this FcntlEvent object.
        """
        event = FcntlEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            FcntlEvent.objects.get(sample=self.sample)
            self.fail('FcntlEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_fcntlevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the FcntlEvent
    ...
    def test_fcntlevent_foreignkey_KernelTrace_delete(self):
        """
        This test checks if the KernelTrace object referenced by the FcntlEvent
