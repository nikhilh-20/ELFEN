...
from analysis.analysis_models.dynamic_analysis import ForkEvent, KernelTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class GetForkEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def setUpTestData(cls):
        ...
        ForkEvent.objects.create(
            sample=cls.sample,
            kernel_trace=cls.kernel_trace,
            procname=b"test"
        )

    def test_forkevent_get(self):
        """
        This test checks if created GetForkEvent object can be retrieved
        successfully from the DB.
        """
        event = ForkEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_forkevent_update(self):
        """
        This test checks if created GetForkEvent object can be updated
        successfully.
        """
        event = ForkEvent.objects.get(sample=self.sample)
    ...
    def test_forkevent_delete(self):
        """
        This test checks if an existing GetForkEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this GetForkEvent object.
        """
        event = ForkEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            ForkEvent.objects.get(sample=self.sample)
            self.fail('GetForkEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_forkevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the GetForkEvent
    ...
    def test_forkevent_foreignkey_KernelTrace_delete(self):
        """
        This test checks if the KernelTrace object referenced by the GetForkEvent
