...
from analysis.analysis_models.dynamic_analysis import UnlinkEvent, KernelTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class UnlinkEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def setUpTestData(cls):
        ...
        UnlinkEvent.objects.create(
            sample=cls.sample,
            kernel_trace=cls.kernel_trace,
            procname=b"test"
        )

    def test_unlinkevent_get(self):
        """
        This test checks if created UnlinkEvent object can be retrieved
        successfully from the DB.
        """
        event = UnlinkEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_unlinkevent_update(self):
        """
        This test checks if created UnlinkEvent object can be updated
        successfully.
        """
        event = UnlinkEvent.objects.get(sample=self.sample)
    ...
    def test_unlinkevent_delete(self):
        """
        This test checks if an existing UnlinkEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this UnlinkEvent object.
        """
        event = UnlinkEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            UnlinkEvent.objects.get(sample=self.sample)
            self.fail('UnlinkEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_unlinkevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the UnlinkEvent
    ...
    def test_unlinkevent_foreignkey_KernelTrace_delete(self):
        """
        This test checks if the KernelTrace object referenced by the UnlinkEvent
