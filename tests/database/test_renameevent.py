...
from analysis.analysis_models.dynamic_analysis import RenameEvent, KernelTrace

from django.db.utils import IntegrityError
from django.core.exceptions import ObjectDoesNotExist


class RenameEventTestCase(TestCase):
    databases = {"elfen"}
    ...
    def setUpTestData(cls):
        ...
        RenameEvent.objects.create(
            sample=cls.sample,
            kernel_trace=cls.kernel_trace,
            procname=b"test"
        )

    def test_renameevent_get(self):
        """
        This test checks if created RenameEvent object can be retrieved
        successfully from the DB.
        """
        event = RenameEvent.objects.get(sample=self.sample)
        self.assertEqual(event.sample.sha256, self.sha256)

    def test_renameevent_update(self):
        """
        This test checks if created RenameEvent object can be updated
        successfully.
        """
        event = RenameEvent.objects.get(sample=self.sample)
    ...
    def test_renameevent_delete(self):
        """
        This test checks if an existing RenameEvent object can be deleted.
        It should be, since *currently* there are no other db objects referencing
        this RenameEvent object.
        """
        event = RenameEvent.objects.get(sample=self.sample)
        event.delete()

        try:
            RenameEvent.objects.get(sample=self.sample)
            self.fail('RenameEvent object not deleted in database')
        except ObjectDoesNotExist:
            pass

    def test_renameevent_foreignkey_sample_cannot_delete(self):
        """
        This test checks if the SampleMetadata object referenced by the RenameEvent
    ...
    def test_renameevent_foreignkey_KernelTrace_delete(self):
        """
        This test checks if the KernelTrace object referenced by the RenameEvent
