...
def get_staticdetection_report(parent_task):
    error_msg = []
    return parent_task.detection.static_analysis_detectors, error_msg


def get_behavioraldetection_report(parent_task):
    error_msg = []
    return parent_task.detection.dynamic_analysis_detectors, error_msg


def get_detection_backend_report(submission_uuid, backend):
    """
    This function retrieves report for a specific backend in the detection
    analysis pipeline.
    ...
    if backend:
        try:
            ...
            # Detection task has not yet started
            return report
